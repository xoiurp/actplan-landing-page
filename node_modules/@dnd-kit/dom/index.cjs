  ...
  constructor(manager, options) {
    ...
    const styleInjectionCleanup = state.effect(() => {
      ...
      if (status.initialized) {
        const sourceDocument = utilities.getDocument((_a3 = source == null ? void 0 : source.element) != null ? _a3 : null);
    ...
    const cleanupEffect = state.effect(() => {
      ...
      let cleanup;
      const frameTransform = utilities.getFrameTransform(element);
      ...
      const actual = new utilities.DOMRectangle(element, {
        ignoreTransforms: true
      });
      const offset = {
      ...
      dropEffectCleanup = state.effect(function dropAnimation() {
        if (dragOperation.status.dropped) {
          ...
          manager.renderer.rendering.then(() => {
            ...
            const sameFrame = utilities.getFrameElement(element) === utilities.getFrameElement(target);
            const options2 = {
              frameTransform: sameFrame ? null : void 0
            };
            const current = new utilities.DOMRectangle(element, options2);
